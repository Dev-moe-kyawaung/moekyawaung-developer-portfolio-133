import { Download, GraduationCap, Briefcase, Award, GitBranch, Star, GitFork,
  BookOpen, Clapperboard, Users, Lightbulb, TrendingUp, Timer, Cpu, Gauge,
  Smartphone, Rows3, Server, Database, Lock, Palette, Ruler, Wifi, Battery } from 'lucide-react';
...
import { PageShell, BlockTitle, NumberedRow, DefCard, TagRow, PageCta } from '../components/pageshell';
    ...
    <PageShell route="tech-stack">
      <div className="space-y-6">
        {STACK_LAYERS.map((l, i) => (
          <Reveal key={l.layer} delay={i * 70}>
            <div className="cyber-card clip-cy p-6 sm:p-7 grid md:grid-cols-[220px_1fr] gap-6 items-center sweep">
              <div className="flex items-center gap-4">
                <span className="w-12 h-12 clip-cy-sm border flex items-center justify-center shrink-0" style={{ color: l.color, borderColor: l.color }}>
                  {[<Gauge key="a" />, <Server key="b" />, <Database key="c" />, <Cpu key="d" />, <Lock key="e" />, <Users key="f" />][i]}
                </span>
                <div>
                  <div className="font-mono2 text-[9px] tracking-[0.2em] text-[var(--faint)]">LAYER {String(i + 1).padStart(2, '0')}</div>
                  <h3 className="font-display font-bold text-base mt-1" style={{ color: l.color }}>{l.layer}</h3>
                </div>
              </div>
              <TagRow items={l.items} color={l.color} />
            </div>
          </Reveal>
        ))}
      </div>
